...
1. **Commit only your own files.** `git status` first — never `git add -A`. Other agents (Claude, Codex) may have unrelated uncommitted WIP in the same working tree; only stage files you actually changed.
