    ...
    <div className="space-y-8">
      <GlassCard className="p-8 md:p-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <p className="text-[10px] uppercase tracking-[0.3em] text-[#D4B47A] font-black">System Health</p>
            <h3 className="text-2xl md:text-3xl font-black uppercase italic mt-2">Live Service Status</h3>
            <p className="text-xs text-zinc-400 mt-3">
              Mode: <span className={healthQuery.data?.mode === 'NORMAL' ? 'text-[#D4B47A]' : 'text-[#D4B47A]'}>{healthQuery.data?.mode || 'UNKNOWN'}</span>
              {' '}• Last refresh: {formatTime(healthQuery.data?.timestamp)}
              {' '}• {stale ? <span className="text-rose-400">STALE</span> : <span className="text-[#D4B47A]">LIVE</span>}
            </p>
          </div>
          ...
          <GlassCard key={key} className="p-6 space-y-4">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                <Icon className="w-5 h-5 text-[#D4B47A]" />
            ...
            {state.status !== 'OK' && (
              <p className="text-[11px] text-white/90 leading-relaxed">{state.next_action || 'Check logs and retry probe.'}</p>
            )}
