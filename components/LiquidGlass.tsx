...
export const GlassCard: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = "" }) => (
  <div className={`card liquid-glass rounded-[32px] border border-[#60A5FA]/22 relative overflow-hidden bg-white/[0.04] shadow-[0_10px_42px_rgba(2,5,14,0.62),0_0_24px_rgba(37,99,235,0.14)] ${className}`}>
...
export const LuxuryFloatingInput: React.FC<{
  ...
  return (
    <div className="relative mb-4 group w-full">
      <motion.div
        animate={{
          borderColor: focused ? 'rgba(96, 165, 250, 0.75)' : error ? 'rgba(244, 63, 94, 0.6)' : isValid ? 'rgba(59, 124, 246, 0.55)' : 'rgba(96, 165, 250, 0.28)',
          backgroundColor: focused ? 'rgba(4, 8, 20, 0.98)' : error ? 'rgba(30, 10, 40, 0.46)' : 'rgba(4, 8, 20, 0.94)',
          boxShadow: focused ? '0 0 0 2px rgba(96, 165, 250, 0.20), 0 10px 26px rgba(2, 5, 14, 0.5)' : '0 8px 18px rgba(2, 5, 14, 0.36)'
        }}
      ...
      >
        <div className={`pl-8 transition-all duration-300 ${focused ? 'text-blue-300 scale-105' : error ? 'text-rose-400' : isValid ? 'text-blue-400' : 'text-zinc-200'}`}>
          {icon}
        </div>
        ...
        <div className="flex-1 relative h-full">
          <motion.label
            ...
            animate={{
              ...
              color: (focused || isFilled) ? '#93C5FD' : error ? '#f43f5e' : isValid ? '#60A5FA' : '#7090B0'
            }}
        ...
        <AnimatePresence>
          {focused && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-gradient-to-r from-blue-600/6 via-transparent to-transparent pointer-events-none"
            />
          )}
        </AnimatePresence>
      </motion.div>
...
export const PrimaryButton: React.FC<{
  ...
  <motion.button
    ...
    className={`interactive-control relative bg-gradient-to-r from-[#1D4ED8] via-[#2563EB] to-[#1D4ED8] rounded-[32px] font-black text-white overflow-hidden transition-all disabled:opacity-40 disabled:cursor-not-allowed group shadow-[0_0_26px_rgba(37,99,235,0.32)] hover:shadow-[0_0_40px_rgba(37,99,235,0.50)] border border-blue-400/25 ${className}`}
  >
...
export const SocialButton: React.FC<{
  ...
  <motion.button
    ...
    whileTap={{ scale: 0.98 }}
    onClick={onClick}
    className="interactive-control w-full h-18 bg-white/[0.06] backdrop-blur-xl rounded-2xl border border-[#60A5FA]/22 flex items-center justify-center gap-4 transition-all group shadow-[0_8px_24px_rgba(0,0,0,0.28)]"
  >
