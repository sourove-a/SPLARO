...
export const GlassCard: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = "" }) => (
  <div className={`card liquid-glass rounded-[32px] border border-[#D4AF7A]/25 relative overflow-hidden bg-white/[0.04] shadow-[0_10px_42px_rgba(4,2,0,0.58),0_0_24px_rgba(196,146,58,0.14)] ${className}`}>
...
export const LuxuryFloatingInput: React.FC<{
  ...
  return (
    <div className="relative mb-4 group w-full">
      <motion.div
        animate={{
          borderColor: focused ? 'rgba(212, 175, 122, 0.72)' : error ? 'rgba(244, 63, 94, 0.6)' : isValid ? 'rgba(196, 160, 80, 0.55)' : 'rgba(212, 175, 122, 0.28)',
          backgroundColor: focused ? 'rgba(14, 9, 3, 0.98)' : error ? 'rgba(50, 10, 10, 0.46)' : 'rgba(14, 9, 3, 0.94)',
          boxShadow: focused ? '0 0 0 2px rgba(212, 175, 122, 0.18), 0 10px 26px rgba(4, 2, 0, 0.5)' : '0 8px 18px rgba(4, 2, 0, 0.36)'
        }}
      ...
      >
        <div className={`pl-8 transition-all duration-300 ${focused ? 'text-amber-300 scale-105' : error ? 'text-rose-400' : isValid ? 'text-amber-400' : 'text-zinc-200'}`}>
          {icon}
        </div>
        ...
        <div className="flex-1 relative h-full">
          <motion.label
            ...
            animate={{
              ...
              color: (focused || isFilled) ? '#D4AF7A' : error ? '#f43f5e' : isValid ? '#C4A050' : '#A89070'
            }}
        ...
        <AnimatePresence>
          {focused && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-gradient-to-r from-amber-600/6 via-transparent to-transparent pointer-events-none"
            />
          )}
        </AnimatePresence>
      </motion.div>
...
export const PrimaryButton: React.FC<{
  ...
  <motion.button
    ...
    whileHover={!disabled && !isLoading ? {
      scale: 1.012,
      y: -2,
      boxShadow: "0 14px 30px rgba(196, 146, 58, 0.42)"
    } : {}}
    whileTap={!disabled && !isLoading ? {
      scale: 0.988,
      y: 1,
      boxShadow: "0 6px 16px rgba(196, 146, 58, 0.28)"
    } : {}}
    onClick={onClick}
    disabled={disabled || isLoading}
    className={`interactive-control relative bg-gradient-to-r from-[#C4923A] via-[#D4A84C] to-[#A87C28] rounded-[32px] font-black text-white overflow-hidden transition-all disabled:opacity-40 disabled:cursor-not-allowed group shadow-[0_0_26px_rgba(196,146,58,0.30)] hover:shadow-[0_0_40px_rgba(196,146,58,0.46)] border border-amber-400/25 ${className}`}
  >
...
export const SocialButton: React.FC<{
  ...
  <motion.button
    ...
    whileTap={{ scale: 0.98 }}
    onClick={onClick}
    className="interactive-control w-full h-18 bg-white/[0.06] backdrop-blur-xl rounded-2xl border border-[#D4AF7A]/25 flex items-center justify-center gap-4 transition-all group shadow-[0_8px_24px_rgba(0,0,0,0.28)]"
  >
