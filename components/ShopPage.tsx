...
const FilterPill: React.FC<{
  ...
  <motion.button
    ...
    className={`relative px-5 py-3 rounded-2xl border transition-all duration-500 flex items-center gap-3 overflow-hidden group ${isSelected
      ? 'bg-green-600/20 border-green-500/50 text-green-400 shadow-[0_0_30px_rgba(52,168,83,0.15)]'
      : 'bg-white/[0.03] backdrop-blur-md border-white/5 text-white/40 hover:border-white/20 hover:text-white'
      }`}
  >
    {isSelected && <motion.div layoutId={`pill-glow-${label}`} className="absolute inset-0 bg-green-500/5 blur-xl pointer-events-none" />}
    {colorHex ? (
      <div className="w-4 h-4 rounded-full border border-white/20 shadow-inner" style={{ backgroundColor: colorHex }} />
    ) : (
      <div className={`w-2 h-2 rounded-full ${isSelected ? 'bg-green-500 scale-125' : 'bg-white/10'}`} />
    )}
  ...
  <motion.button
    ...
    className={`relative w-16 h-16 flex items-center justify-center rounded-2xl border transition-all duration-700 overflow-hidden ${!isAvailable
      ...
      : isSelected
        ...
        : 'bg-white/[0.03] backdrop-blur-xl border-white/10 text-white/40 hover:border-green-500/40 hover:text-white'
      }`}
  >
...
const ActiveFilterPill: React.FC<{ label: string; onRemove: () => void }> = ({ label, onRemove }) => (
  <motion.div
    ...
    className="flex items-center gap-2 px-4 py-2 bg-green-600/10 border border-green-500/20 rounded-full text-[9px] font-black text-green-500 uppercase tracking-widest"
  >
    {label}
      ...
      <div className="mb-10 sm:mb-16">
        ...
        >
          ...
          <div className="max-w-xl">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-3 text-green-500 mb-6">
            ...
            >
              {resolvedHero.titleLines.map((line: string, index: number) => (
                <React.Fragment key={`hero-line-${index}`}>
                  <span className={index === resolvedHero.titleLines.length - 1 ? 'text-green-400' : ''}>{line}</span>
            ...
            {!!resolvedHero.heroCtaLabel && (
              <button
                ...
                className="mt-6 min-h-12 px-6 rounded-full border border-green-400/40 bg-green-600/10 hover:bg-green-600/20 text-green-300 text-[10px] font-black uppercase tracking-[0.2em] transition-all"
              >
                {resolvedHero.heroCtaLabel}
              </button>
            )}
          ...
          {!!activeFilterSet?.filters.find((filter) => filter.id === 'sort') && (
            <div className="flex flex-wrap gap-4 w-full md:w-auto">
              ...
              <button
                ...
                className="lg:hidden min-h-12 px-6 py-3 liquid-glass rounded-full border border-white/10 text-green-400 text-[10px] font-black uppercase tracking-widest flex items-center gap-3 touch-manipulation"
              >
        ...
        >
          <div className={`space-y-8 ${showFilters ? '' : 'lg:sticky lg:top-48'}`}>
            <div className="lg:hidden flex items-center justify-between pb-2 border-b border-white/10">
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-green-300">Filters</h3>
              <button
                type="button"
                onClick={() => setShowFilters(false)}
            ...
            <div className="space-y-6">
              <h4 className="text-[11px] font-black uppercase text-white tracking-[0.4em] flex items-center gap-3 border-b border-white/5 pb-4">
                <Layers className="w-4 h-4 text-green-500" /> Category
              </h4>
              ...
              .map((filter) => {
                ...
                return (
                  <div className="space-y-6" key={filter.id}>
                    <h4 className="text-[11px] font-black uppercase text-white tracking-[0.4em] flex items-center gap-3 border-b border-white/5 pb-4">
                      <Tag className="w-4 h-4 text-green-500" /> {filter.label}
                    </h4>
                    {isSizeFilter(filter) ? (
                      ...
                      filter.id === 'brand' ? (
                        <div className="space-y-3">
                          <button
                            ...
                            className="w-full min-h-12 px-4 rounded-2xl border border-white/10 bg-white/[0.03] text-white/80 hover:border-green-500/40 transition-all flex items-center justify-between"
                          >
                              ...
                              {options.map((option) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <span className={`w-2.5 h-2.5 rounded-full ${isChecked ? 'bg-green-400' : 'bg-white/20'}`} />
                                  </button>
                                );
                              })}
                            </div>
            ...
            {priceFilter && priceRange && (
              <div className="space-y-6">
                <h4 className="text-[11px] font-black uppercase text-white tracking-[0.4em] flex items-center gap-3 border-b border-white/5 pb-4">
                  <SortAsc className="w-4 h-4 text-green-500" /> Price Range
                </h4>
                <div className="grid grid-cols-2 gap-3">
                  <div className="px-4 py-3 rounded-2xl bg-white/[0.03] border border-white/10">
                    ...
                    <input
                      ...
                      className="w-full bg-transparent text-sm font-black text-green-400 outline-none"
                    />
                  </div>
                  <div className="px-4 py-3 rounded-2xl bg-white/[0.03] border border-white/10">
