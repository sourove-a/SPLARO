  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        className={`nav-item interactive-control relative p-2 ${mobile ? 'min-h-12 min-w-12 bg-white/5 backdrop-blur-3xl rounded-[12px] border border-white/10 hover:border-white/50 transition-all shadow-xl' : 'group'}`}
      >
        <div className="w-10 h-10 rounded-xl flex items-center justify-center border border-white/5 bg-white/[0.02] text-white/70 group-hover:text-white group-hover:border-[#FFFFFF]/30 transition-all duration-500">
          <Bell className="w-5 h-5" />
        </div>
        {unread > 0 && (
          <span className="absolute -top-1 -right-1 bg-[#FFFFFF] text-white text-[9px] min-w-5 h-5 rounded-full flex items-center justify-center px-1.5 font-black border border-white/30">
            {unread > 99 ? '99+' : unread}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className={`absolute ${mobile ? 'right-0' : 'right-0'} mt-3 w-[320px] max-w-[92vw] rounded-xl border border-white/10 bg-[#05070e]/95 backdrop-blur-2xl shadow-[0_22px_60px_rgba(0,0,0,0.45)] z-[700]`}>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {!isRead && <span className="w-2 h-2 rounded-full bg-blue-400 mt-1.5 shrink-0" />}
                    </div>
