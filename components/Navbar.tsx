  ...
  >
    ...
    >
      <div className="flex flex-col">
        <span className="text-[10px] font-black text-blue-200/85 uppercase tracking-[0.44em] mb-3 opacity-90 group-hover:opacity-100 transition-all duration-500 transform translate-y-0.5 group-hover:translate-y-0 italic">
          Quick Access
        </span>
      ...
      <motion.div
        ...
        className="w-16 h-16 rounded-full liquid-glass border border-blue-200/35 flex items-center justify-center opacity-90 group-hover:opacity-100 group-hover:border-blue-200/80 transition-all duration-500"
      >
        <ArrowRight className="w-6 h-6 text-blue-100 group-hover:text-white" />
      </motion.div>
    </button>
  </motion.div>
);
          ...
          <div className="hidden lg:flex items-center gap-6 bg-white/5 backdrop-blur-2xl px-6 py-4 rounded-[24px] border border-white/5 shadow-xl pointer-events-auto">
            {leftItems.map((item) => {
              ...
              return (
                ...
                >
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center border border-white/5 bg-white/[0.02] group-hover:bg-blue-600/10 group-hover:border-blue-500/30 transition-all duration-500 ${isActive ? 'text-blue-400 border-blue-500/50 bg-blue-500/5 shadow-[0_0_15px_rgba(37, 99, 235,0.2)]' : 'text-white/40 group-hover:text-white'}`}>
                    <item.icon className="w-5 h-5" />
                  </div>
                  <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all text-[8px] font-black uppercase tracking-[0.2em] text-blue-500 whitespace-nowrap pointer-events-none">
                    {item.label}
                  </div>
                </button>
              );
            ...
            >
              <div className="w-10 h-10 rounded-xl flex items-center justify-center border border-white/5 bg-white/[0.02] group-hover:bg-blue-600/10 group-hover:border-blue-500/30 transition-all duration-500 text-white/40 group-hover:text-white">
          ...
          <div className="hidden lg:flex items-center gap-6 bg-white/5 backdrop-blur-2xl px-6 py-4 rounded-[24px] border border-white/5 shadow-xl pointer-events-auto">
            ...
            {rightItems.map((item) => {
              ...
              return item.label === 'IDENTITY' ? (
                ...
                >
                  <div className={`w-10 h-10 rounded-full border border-white/10 overflow-hidden transition-all duration-500 group-hover:border-blue-500/50 group-hover:shadow-[0_0_15px_#34a853] flex items-center justify-center bg-white/5 ${isActive ? 'border-blue-500 border-2 shadow-[0_0_15px_#34a853]' : ''}`}>
                    {user?.profileImage ? (
                ...
                >
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center border border-white/5 bg-white/[0.02] group-hover:bg-blue-600/10 group-hover:border-blue-500/30 transition-all duration-500 ${isActive ? 'text-blue-400 border-blue-500/50 bg-blue-500/5 shadow-[0_0_15px_rgba(37, 99, 235,0.2)]' : 'text-white/40 group-hover:text-white'}`}>
                    ...
                    {item.label === 'CART' && cart.length > 0 && (
                      <span className="absolute -top-1 -right-1 bg-blue-500 text-white text-[8px] w-5 h-5 rounded-full flex items-center justify-center font-black border border-white/30">{cart.length}</span>
                    )}
                  </div>
                  <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all text-[8px] font-black uppercase tracking-[0.2em] text-blue-500 whitespace-nowrap pointer-events-none">
                    {item.label}
          ...
          >
            ...
            {cart.length > 0 && (
              <motion.span
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className="absolute -top-1 -right-1 bg-blue-500 text-white text-[9px] w-6 h-6 rounded-full flex items-center justify-center font-black border-2 border-white/30"
              >
                {cart.length}
              </motion.span>
            )}
      ...
      <AnimatePresence>
        ...
        {menuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[500] bg-[radial-gradient(circle_at_18%_22%,rgba(37, 99, 235,0.22),transparent_44%),radial-gradient(circle_at_82%_80%,rgba(37, 99, 235,0.18),transparent_40%),linear-gradient(180deg,#060D1C_0%,#050f08_42%,#030a05_100%)] overflow-y-auto overscroll-contain flex flex-col"
          >
            ...
            <div onClick={(e) => e.stopPropagation()} className="mt-auto px-4 sm:px-8 md:px-16 pb-6 pt-6 border-t border-white/5">
              <div className="flex items-center gap-4 bg-blue-500/5 p-4 rounded-2xl border border-blue-500/10">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-500">Session Secure</span>
              </div>
            </div>
          </motion.div>
