...
> **Status Legend:** `[LIVE]` = Production Verified · `[CODED]` = Implemented & Unit-Tested (Awaiting Merchant Keys) · `[PARTIAL]` = Partially Wired · `[STUB]` = UI Prototype / Placeholder · `[PLANNED]` = Future Roadmap (Feature-Flagged)

### Customer-Facing Storefront

- `[LIVE]` Full luxury editorial homepage with cinematic hero slider (WebP responsive renditions)
- `[LIVE]` Collection browsing (Luxury Pret, Festive Edit, Sarees, Unstitched, Accessories)
- `[LIVE]` Product detail with liquid-glass chips, image zoom, size guide, and color swatches
- `[LIVE]` Quick view modal
- `[LIVE]` Smart search with instant results
- `[LIVE]` Wishlist (persistent, synced with account via BFF)
- `[LIVE]` Cart with real-time stock verification (Zustand + server hydrator)
- `[LIVE]` Guest checkout + account checkout (one-tap mobile viewport bar)
- `[LIVE]` WhatsApp live chat integration (storefront direct link)
- `[LIVE]` Order tracking (public, no login required via `SPL-####` & phone)
- `[LIVE]` Customer account portal (order history, addresses, profile)
- `[LIVE]` Google 1-click login via Google Identity Services (GIS)
- `[PLANNED]` Loyalty points engine (`FEATURE_LOYALTY_ENABLED=false`)

### Bangladesh eCommerce & Payments

- `[LIVE]` Cash on Delivery (COD) — primary payment method with delivery zone auto-calculation
- `[CODED]` bKash payment integration (create payment → execute → verify signature)
- `[CODED]` Nagad payment integration
- `[CODED]` SSLCommerz payment gateway (card, net banking, IPN webhook)
- `[LIVE]` Inside Dhaka: ৳60 delivery / Outside Dhaka: ৳120 delivery
- `[LIVE]` Same-day dispatch rules for orders before 2PM
- `[CODED]` SMS order confirmation (Bangladesh phone format normalization & BDBulkSMS client)
- `[PARTIAL]` Automated WhatsApp order dispatch notifications (template ready, provider bridge staged)
- `[LIVE]` Division-level delivery time estimates
- `[LIVE]` Promo code / coupon system with limit validation
- `[LIVE]` Bulk discount support

### Courier Automation

- `[CODED]` Auto-booking to Steadfast Courier API on order confirm (live credentials required; stubs reject fake bookings)
- `[CODED]` Pathao & RedX multi-courier abstraction interfaces
- `[PLANNED]` Paperfly courier adapter
- `[LIVE]` Consignment ID and tracking code attached to order
- `[LIVE]` Customer tracking link generated instantly
- `[CODED]` Failed booking retry queue via BullMQ
- `[LIVE]` Admin manual retry, consignment lookup, and courier switch
...
- `[LIVE]` 1-click direct browser printing of invoices and shipping receipts
- `[CODED]` 80mm thermal receipt printer support (ESC/POS via local daemon)
- `[CODED]` Network printer support (IPP protocol)
- `[CODED]` Print queue with retry logic (BullMQ)
- `[LIVE]` Admin print queue monitor and manual reprint

### Google Sheets Integration

- `[LIVE]` Real-time order sync to Google Sheets via NestJS BullMQ processor
- `[LIVE]` Inventory and customer database synchronization
- `[LIVE]` Revenue summary sheet sync
- `[LIVE]` Manual sync trigger from admin panel
- `[LIVE]` Sheet tab mapping and OAuth scope validator

### AI Features (SPLARO Command)

- `[LIVE]` AI product description generator (OpenAI GPT-4o-mini, dual EN/BN copy)
- `[LIVE]` AI SEO title + meta description generator
- `[LIVE]` AI Command Brain (in-app floating operations assistant with tool permission tiers)
- `[LIVE]` AI daily budget limit & cost tracking (`AGENT_DAILY_COST_LIMIT_USD`)
- `[PARTIAL]` AI abandoned cart recovery messages (prompts ready; scheduled queue staged)
- `[LIVE]` AI sales insights & daily executive brief
- `[PARTIAL]` AI low-stock prediction & replenishment recommendations
- `[LIVE]` AI COD fraud risk signals (unreachable phone, high-return address check)

### Admin Operating System (45+ Bespoke Screens)

- `[LIVE]` Overview executive dashboard with revenue trends and fulfillment ladder
- `[LIVE]` Honest feedback architecture (`toastOk`, `toastFail`, `toastApiSaved` on verified API response only)
- `[LIVE]` Product CRUD with variant matrix, cost prices, and barcode assignments
- `[LIVE]` Inventory valuation ledger with low/out-of-stock filters and CSV export
- `[LIVE]` WMS & Warehouse stock movements across 9 transactional types
- `[LIVE]` Procurement Hub: Purchase Orders, Suppliers, and Goods Received Notes (GRN)
- `[LIVE]` Partner Capital Hub: capital investments, withdrawals, and profit hisab ledger
- `[LIVE]` Order management with fulfillment status workflow and live packing station
- `[LIVE]` Customer CRM with phone search, customer 360, and CSV export
- `[LIVE]` Wholesale B2B inquiry pipeline (5 stages, WhatsApp shortcuts, CSV export)
- `[LIVE]` Security Center: active device sessions killswitch and audit event logs
- `[LIVE]` Media Library: visual folder tree, WebP uploader, and R2/CDN asset picker
- `[LIVE]` Verified Settings Shell with server PATCH verification
- `[LIVE]` 1-Click CSV exports across all major data tables

### Future Roadmap: SaaS & Multi-Vendor (Feature Flagged)

> **Architectural Notice:** SPLARO is deployed as a single-brand quiet luxury eCommerce platform (`splaro.co`). Multi-tenant SaaS capabilities and vendor marketplaces are scaffolded in the data model but **feature-flagged off** for the v1 retail release.
...
**Implemented (API / Data Model):**
- `[CODED]` Subscription schema, plan models, and store tenant resolver.
- `[CODED]` Feature-flag guard (`@RequireFeature`) protecting unreleased routes.

**Future Roadmap (Not in V1 Release):**
- `[PLANNED]` Vendor sub-dashboards and independent vendor catalog submission.
- `[PLANNED]` Multi-vendor commission calculation and automated payout statements.
- `[PLANNED]` Custom domain routing per tenant (`tenant.splaro.co`).
