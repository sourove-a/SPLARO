...
- editorial reference size pills: idle glass/black text; selected black/white; no hover/press jump.
- `prefers-reduced-motion: reduce`: hero autoplay/progress/video warm-up stay stopped; brand ribbon under the hero (`.home-flow-strip`) keeps flowing on Windows only — Mac/Linux reduce stays still for scroll GPU budget.
