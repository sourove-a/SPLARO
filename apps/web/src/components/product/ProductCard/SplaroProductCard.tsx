...
import { Heart, ChevronUp, ChevronDown, Check, Loader2 } from 'lucide-react'
...
export function SplaroProductCard({
  ...
  const displayImage = useMemo(() => {
    ...
    imageGallery,
    showGalleryNav,
    useCssHoverCrossfade,
  ])

  useEffect(() => {
    if (!hovered || imageGallery.length <= 1 || showGalleryNav || useCssHoverCrossfade) {
      setGalleryIndex(0)
      return
    }
  ...
  }, [hovered, imageGallery.length, showGalleryNav, useCssHoverCrossfade])
    ...
    (e: React.MouseEvent) => {
      ...
      if (isShop || isHomepage) {
        ...
        if (onAddToBag) {
          ...
          const variantRef = resolveQuickAddVariant(
            {},
            size,
            color,
          )
          addItem({
            ...
            slug,
            ...(variantRef ? { variantId: variantRef.id } : {}),
            ...(size ? { size } : {}),
            ...(color ? { color } : {}),
          })
          trackAddToCart({
        ...
        >
          <div className="product-shared-media" style={mediaTransition}>
            {useCssHoverCrossfade && hoverSrc ? (
              <div className="splaro-card__img-frame splaro-card__img-frame--crossfade">
                <StorefrontImage
                  src={image}
                  alt={name}
                  profile="card"
                ...
                <StorefrontImage
                  ...
                  aria-hidden
                  className={cn(
                    'splaro-card__img splaro-card__img--hover',
                    imageFit === 'cover'
                      ? 'splaro-card__img--cover'
                      : 'splaro-card__img--contain',
                  )}
        ...
        {inStock ? (
          <m.button
            ...
            className={cn(
              ...
              addedFlash && 'splaro-card__wish--added',
            )}
            ...
            {isShop || isHomepage ? (
              <span className="inline-flex" aria-hidden>
                {adding ? (
                  ...
                  <Check size={isHomepage ? 18 : 16} strokeWidth={2.2} />
                ) : (
                  <BagIcon size={isHomepage ? 18 : 16} strokeWidth={1.37} plus />
                )}
              </span>
            ) : (
              <m.span
