  ...
  return (
    <div className="relative hidden sm:block">
      <button
        ...
        aria-expanded={open}
        title="Display preference only — full UI translation coming soon"
      >
      ...
      {open ? (
        <>
          ...
          <div className="admin-header-popover absolute right-0 top-[calc(100%+0.5rem)] z-50 w-44 overflow-hidden rounded-[16px] py-1">
            {LANGUAGES.map((item) => (
              <button
                key={item.code}
                type="button"
                onClick={() => {
                  setLang(item.code)
                  setOpen(false)
                  toastWarn(`${item.label} selected — admin UI is English-only for now.`, 'lang-ui-only')
                }}
