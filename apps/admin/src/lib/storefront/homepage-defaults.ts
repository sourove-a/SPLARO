...
export function resolveOurStory(input?: Partial<OurStoryConfig>): OurStoryConfig {
  const stories =
    input?.customerStories?.stories?.length
      ? input.customerStories.stories
...
export function visibleCustomerStories(config: OurStoryConfig) {
  return config.customerStories.stories.filter((story) => story.enabled)
}
